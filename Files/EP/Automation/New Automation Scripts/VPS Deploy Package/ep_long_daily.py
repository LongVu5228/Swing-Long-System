...
from notify_discord import notify as _notify_discord
...
def notify(message: str, title: str | None = None, *, color: int = 0x3498DB) -> None:
    """Always print to the terminal (and the tee'd log file) so status is visible
    even without Discord configured, then still forward to Discord (no-ops if
    DISCORD_WEBHOOK_URL is unset)."""
    tag = f"[{title}] " if title else ""
    print(f"{datetime.now(ET).strftime('%H:%M:%S')} {tag}{message}")
    _notify_discord(message, title, color=color)
...
def fetch_day_net_pl(sock: socket.socket, timeout_sec: float = 5.0) -> Optional[float]:
    """$AccountInfo field index 5 = the account's own day net realized P&L --
    same field alextweak.py's log_eod_pnl() reads, used here purely as a
    DAS-side cross-check alongside our own per-position tracking below."""
    send_line(sock, "GET AccountInfo")
    buf = b""
    deadline = time.time() + timeout_sec
    while time.time() < deadline:
...
def expire_unconfirmed_gap_watches(state: dict, now: datetime) -> None:
    ...
    for ticker, watch in list(state["watches"].items()):
        if watch["status"] != WatchStatus.PENDING_GAP_CONFIRM.value or watch["day0"] != today_str:
            continue
        # A late-day (re)start can create this watch AFTER GAP_CONFIRM_GRACE_UNTIL
        # has already passed -- give the just-sent "SB {ticker} Lv1" subscription a
        # few seconds to actually return a quote before trusting an empty bid cache
        # as "no bid available" (which would otherwise falsely look like a halt).
        if time.time() - watch.get("created_ts", 0) < 3.0:
            continue
...
def build_eod_pnl_block(state: dict, closed_today: List[dict]) -> str:
    """Per-position P&L for the EOD Discord digest: today's closures (final
    realized P&L, already booked) plus every still-open position (realized-
    to-date from any banked ladder fills, plus unrealized mark-to-market on
    the shares still held, using the last live price seen)."""
    lines: List[str] = []
    total = 0.0
    ...
    for item in closed_today:
        pl = item["realized_pl"]
        total += pl
        r_txt = f", {item['realized_r']:.2f}R" if item.get("realized_r") is not None else ""
        lines.append(f"- **{item['ticker']}** (closed -- {item['exit_reason']}): ${pl:,.2f}{r_txt}")

    for pos in state["positions"].values():
        if not pos.get("reconciled"):
            continue
        ticker = pos["ticker"]
        banked = pos.get("realized_pl", 0.0)
        shares_rem = pos["shares_remaining"]
        last_price = last_price_cache.get(ticker)
        if last_price is not None and shares_rem > 0:
            unrealized = (last_price - pos["entry_fill"]) * shares_rem
            open_total = banked + unrealized
            total += open_total
            lines.append(
                f"- **{ticker}** (open, {shares_rem}sh @ ${last_price:.2f}): ${open_total:,.2f} "
                f"(banked ${banked:,.2f} + unrealized ${unrealized:,.2f})"
            )
        else:
            total += banked
            lines.append(f"- **{ticker}** (open, {shares_rem}sh, no live price): banked ${banked:,.2f} only")

    if not lines:
        return "No closed or open positions today."
    return "\n".join(lines) + f"\n\n**Total (banked + unrealized):** ${total:,.2f}"


def run_eod_updates(sock: socket.socket, state: dict) -> None:
    ...
    # Per-position P&L digest -- built from pending_history_closes BEFORE it's
    # flushed below, since those entries are only ever queued same-day.
    pnl_block = build_eod_pnl_block(state, state["pending_history_closes"])
    day_net_pl = fetch_day_net_pl(sock)
    das_line = (
        f"**DAS day net P&L (whole account, all strategies):** ${day_net_pl:,.2f}\n\n"
        if day_net_pl is not None else ""
    )
    notify(f"{das_line}{pnl_block}", title="EP Long Daily -- EOD P&L", color=0x9B59B6)
