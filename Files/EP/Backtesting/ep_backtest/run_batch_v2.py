...
def main():
    ...
    combined_trades.to_parquet(os.path.join(outputs_dir, "trades_v2.parquet"), index=False)

    summary_df = summarize_all_v2(combined_trades)
    summary_df.to_csv(os.path.join(outputs_dir, "strategy_summary_v2.csv"), index=False)

    print(f"\nwrote {len(combined_trades)} trade rows across {len(summary_df)} V2 strategies")
    print(f"strategy summary: {os.path.join(outputs_dir, 'strategy_summary_v2.csv')}")
