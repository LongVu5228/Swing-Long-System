...
### Follow-up filter: trigger-day candle color (green vs red)
Quick check on v3's 234 trades: classified each trade by whether its trigger day (the day the qualifying first-hour breakout candle formed) closed green (close>open) or red (close<open), using the daily bar for that date.

Split: 163 green (69.7%), 70 red (29.9%), 1 flat. **Green trigger days meaningfully outperform**: avg_R +0.409 / win rate 21.5% vs red's avg_R -0.127 / win rate 14.3%.

**Tested as an actual filter** (green-only, 163 trades): win 21.5%, avg winner 5.93R, avg loser -1.10R, RR 5.39, **PF 1.473** (up from 1.254), **EV_R 0.409R** (up from 0.242R), **total_R +66.6** (up from +56.7) — a real improvement using fewer, better trades.

**Also partially — not fully — addresses the single-year-dependency problem.** Year-by-year on the green-only subset: 2025 is still the largest contributor (+41.2R) but now only ~62% of the total (down from ~95% on the unfiltered set) — **2020 contributes a real second chunk (+18.7R)**, with small positive contributions from 2023 (+3.8R) and 2026 (+9.8R) too. Still genuinely negative in 2018 (-8.6R), 2021 (-9.3R), 2022 (-9.4R), and 2024 (-4.8R) — not a clean, consistent-every-year result, but "carried by two good years instead of one" is a meaningfully less fragile shape than before.

**Net verdict, unchanged in spirit**: real, explainable, easy-to-add filter (candle color is trivial to check) that improves trade quality and modestly diversifies the source of the edge — worth carrying into any future, properly-validated build of the 2nd attempt — but still not enough by itself to call this validated. Same conclusion as above: needs the full §13 gauntlet before being trusted with real capital.

Also flagged in the same conversation, independent of this specific finding: while investigating one trade (LPSN) the user thought looked wrong, discovered that `daily_bars.pull_ticker_daily_bars()` returns Polygon's split-adjusted prices, which retroactively rescale historical prices to match a stock's *current* share count — confirmed real (a fresh, non-cached re-pull gave identical numbers, so not a stale-cache artifact). By itself this is normal/correct behavior for backtesting. **The open concern**: if a ticker's `adr14` (baked into the trades parquet from an earlier point in this project's timeline) and a *freshly*-pulled daily-bar price series don't share the same split-adjustment snapshot (i.e., a split happened on that ticker sometime between when adr14 was computed and when the fresh pull happened), the ADR-based stop math could be silently scale-inconsistent. Most likely to affect the ad-hoc scratchpad scripts from tonight (which mix fresh pulls with older-computed values) rather than the main `trades_v3b_screen2_corrected.parquet` (generated in one self-consistent batch run). **Not resolved this session** — worth a dedicated check (scan for tickers whose current split-adjustment epoch might not match when their cached data / adr14 was originally computed) before trusting any of tonight's ad-hoc 2nd-attempt numbers too literally. In this specific LPSN case, turned out to be a false alarm (user was looking at the wrong year) — the concern stands independently of that one instance being a non-issue.
...
- **Split-adjustment epoch mismatch, potential**: flagged in §12b while investigating an LPSN trade (turned out to be a false alarm for that specific case — user was looking at the wrong year). Real, still-open concern: a ticker's `adr14` (computed earlier in the project) and a freshly-pulled daily-bar price series may not share the same split-adjustment snapshot if a split occurred on that ticker in between, which would silently make ADR-based stop math scale-inconsistent. Most likely to affect tonight's ad-hoc scratchpad scripts, not the main corrected trades parquet. Not investigated further this session.
- **2nd-attempt trigger-day color filter**: found green-trigger-day trades meaningfully outperform red (§12b) — real, cheap, easy-to-add filter (PF 1.25→1.47, total_R +56.7→+66.6) that also partially diversifies away from the single-year (2025) dependency, though doesn't fully resolve it. Worth carrying into any future proper build of the 2nd attempt; not yet run through the §13 validation gauntlet itself.
